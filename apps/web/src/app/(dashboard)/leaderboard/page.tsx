...
import {
  Trophy, Flame, Star, CheckCircle2, Lock,
  Loader2, Gift, Zap,
} from 'lucide-react';
          ...
          <div className="card-glass rounded-xl p-4 flex flex-col justify-between">
            ...
            {rewardResult ? (
              ...
              <button
                ...
                disabled={!stats.dailyRewardAvailable || rewardMutation.isPending}
                className="flex items-center justify-center gap-1.5 py-2 rounded-lg bg-orange-500/10 border border-orange-500/20 text-orange-400 hover:bg-orange-500/20 text-xs font-medium transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
            ...
            {!stats.dailyRewardAvailable && !rewardResult && (
              <p className="text-xs text-zinc-600">Claimed today</p>
            )}
          </div>
        </div>
      )}
