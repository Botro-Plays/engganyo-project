...
interface PlatformTask {
  ...
  creditPerTask: number; requiresProof: boolean; proofInstructions: string | null;
...
export default function AdminCampaignsPage() {
  ...
  const [tab, setTab] = useState<'pending' | 'platform'>('pending');
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-6 bg-surface-hover rounded-lg p-1 w-fit">
        {(['pending', 'platform'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'pending' ? 'Pending Review' : 'Platform Tasks'}
          </button>
        ))}
      </div>
